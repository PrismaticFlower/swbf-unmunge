                   ...
                   msh::Builders_map& msh_builders)
{
   if (chunk.mn == "ucfb"_mn) {
      tasks.run([&] {handle_ucfb(view_type_as<chunks::Ucfb>(chunk),
                                 file_saver,
                                 tasks,
                                 msh_builders); });
   }
   else if (chunk.mn == "lvl_"_mn) {
      tasks.run([&] {handle_lvl_child(view_type_as<chunks::Child_lvl>(chunk),
                                      file_saver,
                                      tasks,
                                      msh_builders); });
   }
   //Object chunks
      ...
      tasks.run([&] {handle_object(view_type_as<chunks::Object>(chunk),
                                   file_saver,
                                   "GameObjectClass"s); });
      }
   ...
   //World chunks
      ...
      tasks.run([&] {handle_world(view_type_as<chunks::World>(chunk),
                                  tasks,
                                  file_saver); });
   }
   else if (chunk.mn == "plan"_mn) {
      tasks.run([&] {handle_planning(view_type_as<chunks::Planning>(chunk),
                                     file_saver); });
   }
      ...
      tasks.run([&] {handle_path(view_type_as<chunks::Path>(chunk),
                                 file_saver); });
   }
   else if (chunk.mn == "tern"_mn) {
      tasks.run([&] {handle_terrain(view_type_as<chunks::Terrain>(chunk),
                                    file_saver); });
   }
   //Model chunks
   else if (chunk.mn == "skel"_mn) {
      tasks.run([&] {handle_skeleton(view_type_as<chunks::Skeleton>(chunk),
                                     msh_builders); });
   }
   else if (chunk.mn == "modl"_mn) {
      tasks.run([&] {handle_model(view_type_as<chunks::Model>(chunk),
                                  msh_builders,
                                  tasks); });
   }
      ...
      tasks.run([&] {handle_collision(view_type_as<chunks::Collision>(chunk),
                                      msh_builders); });
   }
   else if (chunk.mn == "prim"_mn) {
      tasks.run([&] {handle_primitives(view_type_as<chunks::Primitives>(chunk),
                                       msh_builders); });
   }
   //Misc chunks
   else if (chunk.mn == "Locl"_mn) {
      tasks.run([&] {handle_localization(view_type_as<chunks::Localization>(chunk),
                                         tasks,
                                         file_saver); });
   }
   else {
      tasks.run([&] {handle_unknown(chunk,
                                    file_saver); });
   }
}
