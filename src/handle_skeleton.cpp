...
auto read_str_array(const Str_array& names, std::size_t count) -> std::vector<std::string_view>
{
...
void handle_skeleton(const chunks::Skeleton& skel,
                     msh::Builders_map& builders)
{
   ...
   const std::uint16_t bone_count = view_type_as<std::uint16_t>(skel.bytes[name.length() + 1]);
   ...
   while (head < end) {
      ...
      if (chunk.mn == "NAME"_mn) {
         ...
         positions = read_xframe_array(view_type_as<Xframe_array>(skel.bytes[head]), bone_count);
      }
