...
struct Hub_info
{
   ...
   {
      buffer += "Hub(\""_sv; buffer += name; 
      buffer += "\")\n{\n"_sv;

      buffer += "\tPos("_sv; 
      buffer += std::to_string(x); buffer += ", "_sv;
      buffer += std::to_string(y); buffer += ", "_sv;
      buffer += std::to_string(z); buffer += ");\n"_sv;
      buffer += "\tRadius("_sv;
      buffer += std::to_string(radius); buffer += ");\n}\n\n"_sv;
   }
};
...
struct Connection_info
{
   ...
   void write_to_buffer(std::string& buffer, const std::vector<Hub_info>& hubs) const
   {
      ...
      buffer += "Connection(\""_sv; buffer += name; 
      buffer += "\")\n{\n"_sv;

      buffer += "\tStart(\""_sv;
      buffer += hubs[start].name; buffer += "\");\n"_sv;
      buffer += "\tEnd(\""_sv;
      buffer += hubs[end].name; buffer += "\");\n"_sv;
      buffer += "\tFlags("_sv;
      buffer += std::to_string(filter_flags); buffer += ");\n"_sv;
...
auto handle_node(const Node& node, std::uint32_t hub_count) -> std::vector<Hub_info>
{
   ...
   while (head < end) {
      hubs.emplace_back(read_hub(view_type_as<Hub>(node.bytes[head]), 
                                 hub_count, head));
   }
...
auto handle_arcs(const Arcs& arcs, std::uint32_t arc_count) -> std::vector<Connection_info>
{
...
void write_planning(std::string name,
                    const std::vector<Hub_info> hubs,
...
void handle_planning(const chunks::Planning& plan,
                     File_saver& file_saver)
{
   std::uint32_t head = 0;
   ...
   write_planning("ai_paths_"s + std::to_string(plan_count.fetch_add(1)),
                  std::move(hubs), 
